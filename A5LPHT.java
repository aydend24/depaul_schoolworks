...
public class A5LPHT<Key, Value> {
	private static final int INIT_CAPACITY = 4; // 初始容量
...
//        StdOut.println("Found null so \"" + key + "\" is not in the hash table");
	...
	/*
	 * TODO:add a new public method to each. Add the method averageClusterSize() to
	 * A5LPHT This will compute and return the average of the sizes of the clusters
	 * in the table.
	 */

	public int averageClusterSize() {

		// initialize the number and size of clusters
		int numOfClusters = 0;
		int sizeOfClusters = 0;

		// go through the LPHT
		for (int i = 0; i < m; i++) { // m is the size of linear probing table (LPHT)
			// If there are elements in both position i and position i+1, then they are in
			// the same cluster
			if (keys[i] != null && keys[(i + 1) % m] != null) {
				sizeOfClusters++; // Increase the size of clusters
			} else if (sizeOfClusters > 0) {
				numOfClusters++; // Increase the number of clusters
				sizeOfClusters++; // Increase the size of clusters
			}
		}

		if (sizeOfClusters > 0) {
			// If there is an element in the last position, an additional cluster is needed
			numOfClusters++;
		}
		if (numOfClusters == 0) {
			return 0;
		...
		int averageClusterSize = sizeOfClusters / numOfClusters;
